...
class rn41_bt_uart : public uart_interface
{
public:
    logger(uart_interface& impl) : uart_impl(impl) {}

    virtual void init(const char* bt_name, const char* auth_key = "1234", bool remote_config = false)
    {
        if (connected()) return;
    ...
    {
        return uart_impl.read(buffer, byte_count, peek);
    }
